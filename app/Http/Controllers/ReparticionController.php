...
class ReparticionController extends Controller
{
    ...
    {
        $item = Reparticion::select('id', 'Reparticion','Descripcion')->whereNull('deleted_at');

        return Datatables::of($item)
            ->addIndexColumn()
            ->addColumn('action', function ($p) {
                return '<a class="btn btn-info btn-xs btn-datatable-Reparticion" id="' . $p->id . '"><i class="fa fa-bars"></i> ' . 'Detalles' . '</a> &nbsp;';
            })
            ->editColumn('id', '{{$id}}')
            ->make(true);
    }
    ...
    {
        try {
            $item = Reparticion::findOrFail($request->id);
            $data = array(
                'success' => true,
                'data' => $item,
                'msg' => trans('messages.listed')
    ...
    {
        $item = Reparticion::where('id', $request->id)->first();
