...
class EscalafonController extends Controller
{
    ...
    {
        $item = Escalafon::select('id', 'Escalafon','Descripcion')->whereNull('deleted_at');

        return Datatables::of($item)
            ->addIndexColumn()
            ->addColumn('action', function ($p) {
                return '<a class="btn btn-info btn-xs btn-datatable-Escalafon" id="' . $p->id . '"><i class="fa fa-bars"></i> ' . 'Detalles' . '</a> &nbsp;';
            })
            ->editColumn('id', '{{$id}}')
            ->make(true);
    }
    ...
    {
        try {
            $item = Escalafon::findOrFail($request->id);
            $data = array(
                'success' => true,
                'data' => $item,
                'msg' => trans('messages.listed')
    ...
    public function store(EscalafonRequest $request)
    {
        ...
        $item->Escalafon = $request->Escalafon;
    ...
    {
        $item = Escalafon::where('id', $request->id)->first();
